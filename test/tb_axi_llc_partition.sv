...
module tb_axi_llc #(
  ...
  localparam int unsigned PartitionSizeWidth = $clog2(TbNumLines);
  typedef logic [PartitionSizeWidth-1:0] cfg_partition_size_t;
  typedef cfg_partition_size_t [TbMaxPartition-1:0] cfg_partition_sizes_t;
  ...
  initial begin : proc_sim_crtl
    ...
    if (TbCachePartition) begin
      automatic cfg_partition_sizes_t sizes = '0;

      // TODO: Some way to test that the memory writes/reads are in the correct
      // partition. Might need to expose some way in axi_master.run to ask
      // for specific partitions (i.e. user bits) instead of completely random.
      // Or some way to query whether or not partitions are flushed

      // The idea behind this test is that if we make it so all memory goes
      // into partitions 7 and 10, then flushing those partitions and those
      // partitions alone should make the two scoreboards compare equal.
      $info("Random read and write for specific partition regions.");
      // FIXME: there's no way to configure one partition as the whole cache
      // In fact, if we use two partitions to cover 256 lines, then we get this
      // warning: `Request address 0 not mapped, port 0, expect random write or read behavior!`
      sizes = '0;
      sizes[7] = TbNumLines - 1;
      // sizes[10] = 1;
      cache_partition(reg_conf_driver, sizes);
      axi_master.run(TbNumReads, TbNumWrites);
      flush_certain_partition(reg_conf_driver, 7);
      // flush_certain_partition(reg_conf_driver, 10);
      // FIXME: This doesn't work, we have no useful way to test this.
      // compare_mems(cpu_scoreboard, mem_scoreboard);
      clear_spm_cpu(cpu_scoreboard);

      // This test makes sure that flush_all_set works properly (which works
      // by writing `MaxPartitions + 1` to CfgFlushPartition, essentially)
      $info("Random read and write for whole cache region.");
      for (int unsigned i = 0; i < TbMaxPartition; i++) begin
        sizes[i] = TbNumLines / TbMaxPartition;
      end
      axi_master.run(TbNumReads, TbNumWrites);
      ...
      // This one makes sure that flushing each partition individually also works
      $info("Random read and write for whole cache region.");
      axi_master.run(TbNumReads, TbNumWrites);
      for (int unsigned i = 0; i < TbMaxPartition; i++) begin
        flush_certain_partition(reg_conf_driver, i);
      end
      compare_mems(cpu_scoreboard, mem_scoreboard);
      clear_spm_cpu(cpu_scoreboard);
  ...
  task cache_partition(regbus_conf_driver_t reg_conf_driver, cfg_partition_sizes_t partition_sizes);
    automatic logic       cfg_error;
    localparam llc_cfg_addr_e[3:0] addresses = '{CfgSetPartitionHigh1, CfgSetPartitionLow1, CfgSetPartitionHigh0, CfgSetPartitionLow0};

    localparam int unsigned PartitionSizesPerReg = 32 / PartitionSizeWidth;
    if (TbRegWidth % PartitionSizeWidth != 0) begin
      $fatal("code assumes PartitionSizeWidth (%d) fits integer multiples in RegWidth (%d)", PartitionSizeWidth, TbRegWidth);
    end

    $info("Configuring partition size!");

    for (int unsigned r = 0; r < TbMaxPartition / PartitionSizesPerReg; r++) begin
      automatic logic [31:0] data = '0;
      automatic llc_cfg_addr_e reg_addr = addresses[r];

      for (int unsigned i = 0; i < PartitionSizesPerReg; i++) begin
        automatic int unsigned p = r * PartitionSizesPerReg + i;
        if (p == TbMaxPartition) break;

        data |= partition_sizes[p] << (i * PartitionSizeWidth);
      end

      reg_conf_driver.send_write(reg_addr, data, 4'hF, cfg_error);
    end

    reg_conf_driver.send_write(CommitPartitionCfg, 32'd1, 4'hF, cfg_error);
